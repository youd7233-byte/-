  ...
  if (isClient) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        {/* Client Welcome */}
        <div style={{
          background: "linear-gradient(135deg, #1A1208 0%, #231810 50%, #1A1208 100%)",
          ...
          <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, left: 0, background: "radial-gradient(ellipse 60% 80% at 90% 50%, rgba(201,168,76,0.08) 0%, transparent 70%)", pointerEvents: "none" }} />
          <h1 style={{ fontSize: "1.7rem", fontWeight: 900, marginBottom: "0.35rem", color: "#E2BE6E" }}>
            مرحباً، {user.name}! 👋
          </h1>
          <p style={{ opacity: 0.7, fontWeight: 600, color: "#B8A070" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }} className="client-actions-grid">
          {[
            { href: "/dashboard/map", icon: "📍", title: "ابحث في الخريطة", desc: "اكتشف الحرفيين القريبين منك", color: "#C9A84C" },
            ...
            <Link key={action.href} href={action.href} style={{
              ...
              textDecoration: "none", color: "#F0E6C8",
              transition: "all 0.2s",
            }}>
              <div style={{
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: "1.05rem", color: "#F0E6C8" }}>{action.title}</div>
                <div style={{ fontSize: "0.85rem", color: "#6A5538" }}>{action.desc}</div>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
            <h2 style={{ fontSize: "1.1rem", fontWeight: 900, color: "#F0E6C8" }}>
            ...
            {topRankedArtisans.map((artisan, i) => (
              <Link key={artisan.userId} href={`/artisan/${artisan.userId}`} style={{
                ...
                <div style={{
                  ...
                  background: artisan.image ? `url(${artisan.image}) center/cover` : "linear-gradient(135deg, #C9A84C, #6B4E1A)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: "#0A0806", fontWeight: 900, fontSize: "1.2rem",
                  boxShadow: "0 2px 12px rgba(201,168,76,0.25)",
                }}>
                  {!artisan.image && artisan.name.charAt(0)}
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontWeight: 800, fontSize: "0.95rem", color: "#F0E6C8" }}>{artisan.name}</div>
                  <div style={{ fontSize: "0.8rem", color: "#C9A84C", fontWeight: 700 }}>{artisan.profession}</div>
  ...
  return (
    <div className="artisan-dash" style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{
        ...
        <div style={{
          height: "120px",
          background: "linear-gradient(135deg, #1A1208 0%, #2A1A08 50%, #C9A84C22 100%)",
          borderBottom: "1px solid rgba(201,168,76,0.15)",
          position: "relative",
        }}>
          <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 80% 100% at 80% 50%, rgba(201,168,76,0.12) 0%, transparent 70%)" }} />
          {profile.isPremium && (
            <div style={{
              position: "absolute", top: "1rem", left: "1rem",
              background: "linear-gradient(135deg, #C9A84C, #8B6B2A)", color: "#0A0806",
        ...
        <div style={{ padding: "0 2rem 1.5rem", position: "relative" }}>
          <div style={{
            ...
            background: user.image ? `url(${user.image}) center/cover` : "linear-gradient(135deg, #C9A84C, #6B4E1A)",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.75rem" }}>
            <div>
              ...
              <div style={{ fontSize: "0.9rem", color: "#C9A84C", fontWeight: 700, marginBottom: "0.25rem" }}>
                {profile.profession}
              </div>
              <div style={{ fontSize: "0.85rem", color: "#6A5538", fontWeight: 600, display: "flex", alignItems: "center", gap: "0.35rem" }}>
              ...
              {avgRating && (
                <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", marginTop: "0.4rem" }}>
                  ...
                  <span style={{ color: "#6A5538", fontSize: "0.82rem" }}>({profile.reviews.length} تقييم)</span>
                </div>
              )}
            </div>
      ...
      <div className="artisan-stats-grid" style={{
        ...
        {[
          { icon: "⭐", label: "التقييم", value: avgRating || "—", sub: "من 5", color: "#C9A84C" },
          ...
          <div key={stat.label} style={{
            ...
            <div style={{ fontSize: "0.78rem", color: "#B8A070", fontWeight: 800, marginTop: "0.2rem" }}>{stat.label}</div>
            <div style={{ fontSize: "0.7rem", color: "#6A5538", marginTop: "2px" }}>{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }} className="artisan-actions-grid">
        {[
          { href: "/dashboard/map", icon: "📍", title: "خريطة الحرفيين", desc: "اكتشف المنطقة وزملائك", color: "#C9A84C" },
          ...
          <Link key={action.href} href={action.href} style={{
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: "0.95rem", color: "#F0E6C8" }}>{action.title}</div>
              <div style={{ fontSize: "0.8rem", color: "#6A5538" }}>{action.desc}</div>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }} className="artisan-bottom-grid">
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: "1.05rem", fontWeight: 900, color: "#F0E6C8", marginBottom: "1rem" }}>
            💬 آخر التعليقات والتقييمات
          </h2>
          {profile.reviews.length === 0 ? (
            <div style={{ textAlign: "center", padding: "1.5rem", color: "#6A5538" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {profile.reviews.slice(0, 3).map((r) => (
                <div key={r.id} style={{
                  padding: "0.85rem", background: "rgba(201,168,76,0.05)",
                  borderRadius: "12px", borderRight: "3px solid rgba(201,168,76,0.4)",
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem" }}>
                    <div style={{
                      ...
                      display: "flex", alignItems: "center", justifyContent: "center",
                      color: "#0A0806", fontWeight: 900, fontSize: "0.8rem", flexShrink: 0,
                    }}>{r.client.name.charAt(0)}</div>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: "0.85rem", color: "#F0E6C8" }}>{r.client.name}</div>
                      <div style={{ fontSize: "0.75rem", color: "#C9A84C" }}>{"★".repeat(r.rating)}</div>
                    </div>
                    <div style={{ marginRight: "auto", fontSize: "0.72rem", color: "#6A5538" }}>
                      {new Date(r.createdAt).toLocaleDateString("ar-DZ")}
                    </div>
                  </div>
                  {r.comment && (
                    <p style={{ color: "#B8A070", fontSize: "0.82rem", lineHeight: 1.6, margin: 0 }}>
                      {r.comment}
                    </p>
                  )}
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {topRanked.map((artisan, i) => (
              <Link key={artisan.id} href={`/artisan/${artisan.userId}`} style={{
                ...
                borderRadius: "12px",
                background: artisan.userId === user.id ? "rgba(201,168,76,0.1)" : "transparent",
                border: artisan.userId === user.id ? "1.5px solid rgba(201,168,76,0.25)" : "1.5px solid transparent",
                transition: "all 0.2s",
              }}>
                <div style={{
                  width: "28px", height: "28px", borderRadius: "50%",
                  background: i === 0 ? "linear-gradient(135deg, #C9A84C, #8B6B2A)" : i === 1 ? "#7A7A7A" : i === 2 ? "#8B5E3C" : "rgba(201,168,76,0.15)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontWeight: 900, fontSize: "0.82rem",
                  color: i < 3 ? "#0A0806" : "#C9A84C", flexShrink: 0,
                }}>{i + 1}</div>
                <div style={{
                  width: "36px", height: "36px", borderRadius: "50%",
                  background: artisan.image ? `url(${artisan.image}) center/cover` : "linear-gradient(135deg, #C9A84C, #6B4E1A)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: "#0A0806", fontWeight: 900, fontSize: "0.9rem", flexShrink: 0,
                }}>
                  {!artisan.image && artisan.name.charAt(0)}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 800, fontSize: "0.88rem", color: "#F0E6C8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {artisan.name} {artisan.userId === user.id && "(أنت)"}
                  </div>
                  <div style={{ fontSize: "0.75rem", color: "#C9A84C", fontWeight: 700 }}>{artisan.profession}</div>
                </div>
            ...
            {topRanked.length === 0 && (
              <p style={{ color: "#6A5538", textAlign: "center", padding: "1rem" }}>لا يوجد بيانات بعد</p>
            )}
          </div>
        </div>
      </div>
      ...
      {!profile.lat && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: "0.85rem", color: "#8B6B2A" }}>
