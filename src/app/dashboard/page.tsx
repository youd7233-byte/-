      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "2.5rem 1.5rem", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem", marginBottom: "2rem" }}>
          <Link href="/complete-profile" style={{
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: "1.05rem" }}>تعديل ملفي</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {profile.reviews.slice(0, 3).map((r) => (
                <div key={r.id} style={{
                  ...
                  borderRadius: "14px",
                  borderRight: "4px solid var(--terracotta)",
                }}>
                  <div style={{ display: "flex", gap: "0.25rem", marginBottom: "0.5rem" }}>
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span key={i} style={{ color: i < r.rating ? "#f59e0b" : "#e5e7eb" }}>★</span>
                    ))}
                  </div>
                  {r.comment && <p style={{ color: "var(--text)", fontSize: "0.95rem", lineHeight: 1.6 }}>{r.comment}</p>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: "0.88rem", color: "#b45309" }}>
                أضف موقعك لكي يتمكن العملاء القريبون منك من إيجادك على الخريطة
              </p>
            </div>
            <Link href="/complete-profile" style={{
